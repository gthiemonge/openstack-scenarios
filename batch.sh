...
curl -X PUT \
    ...
    -d '{"members":[{"name": "member1", "subnet_id": "'$subnet_id'","address":"192.0.1.1","protocol_port":80},{"name": "member2", "subnet_id": "'$subnet_id'","address":"192.0.1.2","protocol_port":80}]}' \
...
curl -X PUT \
    -H "Content-Type: application/json" \
    -H "X-Auth-Token: $TOKEN" \
    -d '{"members":[{"name": "member3", "subnet_id": "'$subnet_id'","address":"192.0.1.3","protocol_port":80},{"name": "member4", "subnet_id": "'$subnet_id'","address":"192.0.1.4","protocol_port":80},{"name": "member1", "subnet_id": "'$subnet_id'","address":"192.0.1.1","protocol_port":80}]}' \
    ${OCTAVIA_BASE_URL}/v2.0/lbaas/pools/$pool_id/members?additive_only=True
echo
wait_for_lb lb1
