...
silent cmd \
    openstack loadbalancer pool create --name pool1 --lb-algorithm ROUND_ROBIN --listener listener1 --protocol UDP --session-persistence type=SOURCE_IP
wait_for_lb lb1

#silent cmd \
#    openstack loadbalancer healthmonitor create --delay 5 --max-retries 4 --timeout 10 --type UDP --name healthmonitor1 pool1
#
#silent cmd \
